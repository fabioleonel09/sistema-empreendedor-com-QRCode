...
namespace sistemaGerenciamentoEmpreendedor
{
    public partial class frmGerenciamento : Form
    {
        ...
        private void btnSair_Click(object sender, EventArgs e)
        {
            ...
            timer2.Stop(); //para o segundo timer
        ...
        //ao clicar no rbqrcode da loja
        private void rbQRCodeLoja_CheckedChanged(object sender, EventArgs e)
        {
            if (rbQRCodeLoja.Checked == true)
            {
            ...
            //inabilita e habilita os pb's
            pbQRCodeLoja.Visible = true;
            pbQRCodeConsig.Visible = false;
        ...
        //ao clicar no rbqrcode do consignado
        private void rcQRCodeConsig_CheckedChanged(object sender, EventArgs e)
        {
            if (rcQRCodeConsig.Checked == true)
            {
                captureDevice = new VideoCaptureDevice(filterInfoCollection[cboDevice.SelectedIndex].MonikerString); //instancia a captura da cam escolhida no comboBox
                captureDevice.NewFrame += CaptureDevice_NewFrame; //associa a cam escolhida
                captureDevice.Start(); //inicia a cam
                timer2.Start(); //inicia o timer
            }

            else
            {
                if (rcQRCodeConsig.Checked == false)
                {
        ...
        //aqui pega-se a leitura dos qrcodes, passa para os txt's e busca no banco de dados
        private void timer1_Tick_1(object sender, EventArgs e)
        {
            if (pbQRCodeLoja.Image != null) //caso o pictureBox esteja com alguma imagem capturada
            {
                BarcodeReader barcodeReader = new BarcodeReader(); //instancia a leitura do qrcode
                Result result = barcodeReader.Decode((Bitmap)pbQRCodeLoja.Image); //verifica se o resultado corresponde a um qrcode

                if (result != null) //se o resultado for ok
                {
                    txtCapCamLoja.Text += result.ToString(); // concatena a leitura do qrcode e converte para o txt ler
                   
                    som = new SoundPlayer(Properties.Resources.efeitoSonoro); //instancia o efeito sonoro
                    som.Play(); //ativa o efeito sonoro

                    pbQRCodeLoja.Image = null; //ataliza a pictureBox
                    timer1.Stop(); //para o timer
        ...
        //implementa o btn para parar a cam
        private void btnPararCam_Click(object sender, EventArgs e)
        {
            pbQRCodeLoja.Image = null; //ataliza a pictureBox
            pbQRCodeConsig.Image = null; //atualiza a pctureBox
        ...
        //evento do segundo timer
        private void timer2_Tick(object sender, EventArgs e)
        {
            if (pbQRCodeConsig.Image != null) //caso o pictureBox esteja com alguma imagem capturada
            {
                BarcodeReader barcodeReader = new BarcodeReader(); //instancia a leitura do qrcode
                Result result = barcodeReader.Decode((Bitmap)pbQRCodeConsig.Image); //verifica se o resultado corresponde a um qrcode

                if (result != null) //se o resultado for ok
                {
                    txtCapCamConsig.Text += result.ToString(); // concatena a leitura do qrcode e converte para o txt ler
